...
using ModuleErrorCode = Testflow.CoreCommon.ModuleErrorCode;
...
namespace Testflow.SlaveCore.Runner.Expression
{
    public class ExpressionParser
    {
        ...
        public ExpressionParser(ExpressionOperatorInfo[] operatorInfos)
        {
            ...
            foreach (ExpressionOperatorInfo operatorInfoPair in operatorInfos)
            {
                string operatorSymbol = operatorInfoPair.Symbol;
            ...
            _operatorAdapters = new List<OperatorAdapter>(operatorInfos.Length);
            // 正则表达式中的元符号
            ...
            {
                OperatorAdapter operatorAdapter = new OperatorAdapter(operatorInfo, argumentPattern, metaCharacters);
...
//            return argumentPattern.ToString();
        ...
        public IExpressionData ParseExpression(string expression, ISequenceStep step)
        {
            ...
            ParsingPostProcess(expressionData, step, _argumentCache);
        ...
        private IExpressionData ParseExpressionData(StringBuilder expressionCache)
        {
            ...
            while (!_parseOverRegex.IsMatch(oldExpression))
            {
                ...
                {
                    throw new TestflowDataException(ModuleErrorCode.ExpressionError, "IllegalExpression");
                }
        ...
        private void ParsingPostProcess(IExpressionData expressionData, ISequenceStep step,
            Dictionary<string, string> argumentCache)
        {
            ExpressionPostProcess(expressionData, argumentCache, step);
        }

        private void ExpressionPostProcess(IExpressionData expressionData, Dictionary<string, string> argumentCache,
            ISequenceStep step)
        {
            ExpressionElementPostProcess(expressionData.Source, argumentCache, step);
            foreach (IExpressionElement expressionElement in expressionData.Arguments)
            {
                ExpressionElementPostProcess(expressionElement, argumentCache, step);
            }
        }

        private void ExpressionElementPostProcess(IExpressionElement expressionElement,
            Dictionary<string, string> argumentCache, ISequenceStep step)
        {
            ...
            {
                ExpressionPostProcess(expressionElement.Expression, argumentCache, step);
                return;
            }
